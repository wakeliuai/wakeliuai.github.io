...
def main():
    ...
    if args.monitor:
            ...
            while True:
                try:
                    ...
                    for message in messages.messages:
                        # Convert object to dict to safely use .get()
        ...
        # List recent messages
        try:
            messages_resp = client.inboxes.messages.list(
                inbox_id=args.inbox,
                limit=args.limit
            )
            
            messages = [m.__dict__ for m in messages_resp.messages] if hasattr(messages_resp, 'messages') else []
            
            if not messages:
                print(f"📭 No messages found in {args.inbox}")
                return
            
            print(f"📧 Messages in {args.inbox} (showing {len(messages)}):\n")
